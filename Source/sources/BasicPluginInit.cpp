...
//    AVAppSetPreference("RESP", !AVAppGetPrefBool("RESP", "Collate", true));
  ...
  {
    ASInt32 type = AVAlert(ALERT_QUESTION, "Please select device for which you want to create layout", "Mobile", "Cancel", "Tablet", false);
    if (type == 3)
      return;

    CreatePagesWithCurrentContent(pdDoc, t_Mobile);
  ...
  if (CosObjGetType(cosLayouts) != CosArray)
    return;

  if (CosArrayLength(cosLayouts) != 1)
    return;
  ...
  for (int i = 0; i < PDDocGetNumPages(pdDoc); i+=2)
  {
    ...
    for (int j = 0; j < PDPageGetNumAnnots(pdPage); j++)
    {
      ...
      if (PDAnnotGetSubtype(annot) == ASAtomFromString("Link"))
      {
        ...
        PDAction action = PDLinkAnnotGetAction(annot);
        ...
        if (PDViewDestIsValid(dest))
        {
          ...
          if (pageNum == i)
          {
            ...
            PDAnnot annot2 = PDPageAddNewAnnot(pdPage2, PDPageGetNumAnnots(pdPage2)-1, ASAtomFromString("Link"), &rect);
            PDAction action2 = PDActionNewFromDest(pdDoc2, viewDest2, pdDoc2);
            PDLinkAnnotSetAction(annot2, action2);
...
ACCB1 ASBool ACCB2 PluginInit(void)
{
  ...
  DURING
    ...
    menuItem = AVMenuItemNew ("Add &New Layout...", btnAddFromCurrent, NULL, true, NO_SHORTCUT, 0, NULL, gExtensionID);
    ...
    menuItem = AVMenuItemNew ("Add New Layout From &File...", btnAddFromFile, NULL, true, NO_SHORTCUT, 0, NULL, gExtensionID);
    ...
    //delete layout
    menuItem = AVMenuItemNew ("Re&move Layout...", btnDelete, NULL, true, NO_SHORTCUT, 0, NULL, gExtensionID);
